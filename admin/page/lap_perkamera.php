...
<form class="form-inline hidden-print" action="<?=$_SERVER["REQUEST_URI"]?>" method="post">
    <label>Kamera :</label>
    <select class="form-control" name="id_kameraku">
        <option>---</option>
        <?php $query = $connection->query("SELECT * FROM kameraku"); while ($r = $query->fetch_assoc()): ?>
            <option value="<?=$r["id_kameraku"]?>"><?=$r["nama_kamera"]?> | <?=$r["no_kamera"]?></option>
        <?php endwhile; ?>
    </select>
    <label>Tgl :</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id_kameraku = isset($_POST['id_kameraku']) ? intval($_POST['id_kameraku']) : 0;
    $start = isset($_POST['start']) ? $_POST['start'] : '';
    $stop = isset($_POST['stop']) ? $_POST['stop'] : '';

    if ($id_kameraku > 0 && !empty($start) && !empty($stop)) {
        $query = "SELECT d.no_kamera, d.nama_kamera, d.merk FROM transaksi a
                  JOIN pelanggan b USING(id_pelanggan)
                  JOIN kameraku d USING(id_kameraku)
                  WHERE d.id_kameraku=$id_kameraku AND a.tgl_sewa BETWEEN '$start' AND '$stop'";
                        ...
                        <form class="form-inline">
                            <table>
                                <tr>
                                    ...
                                    <td>&nbsp;:&nbsp;
                                        <input type="text" value="<?= $row['no_kamera'] ?>" class="form-control" disabled="on"><br>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    ...
                                    <td>&nbsp;:&nbsp;
                                        <input type="text" value="<?= $row['nama_kamera'] ?>" class="form-control" disabled="on"><br>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
...
<?php
        ...
        echo "Harap pilih kameraku dan isi tanggal dengan benar.";
    }
}
?>
